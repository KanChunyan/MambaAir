...
from torch.optim import AdamW  # 确保导入AdamW
import os
import time
import warnings
import numpy as np
...
class Exp_Imputation(Exp_Basic):
    ...
    def _select_optimizer(self):
        # 分离参数：amp_gamma 和 其他参数
        amp_params = [p for n, p in self.model.named_parameters() if "amp_gamma" in n]
        base_params = [p for n, p in self.model.named_parameters() if "amp_gamma" not in n]
        
        # 配置差异化学习率
        optimizer = AdamW(
            [
                {"params": base_params, "lr": self.args.learning_rate},
                {"params": amp_params, "lr": 1e-3}
            ],
            weight_decay=0.01
        )
        return optimizer
